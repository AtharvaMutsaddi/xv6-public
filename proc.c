...
int 
clone(void(*fn)(void*), void *arg, void* threadstack)
{
  struct proc *np;
  ...
  *(uint*)threadret = 0xFFFFFFF;
  threadarg = threadstack + PGSIZE - 1 * sizeof(void *);
  *(uint*)threadarg = (uint)arg;
  np->tf->esp = (uint) threadstack;
  np->threadstack = threadstack;
